            ...
            <div class="col-lg-6">
                ...
                <div class="d-flex align-items-center">
                    ...
                    <div class="d-flex p-1 rounded-pill border"
                        ...
                        <button type="button" @click="trangThaiFilter = 1"
                            class="btn btn-sm rounded-pill w-50 fw-bold border-0 text-nowrap me-2"
                            :class="trangThaiFilter === 1 ? 'bg-white shadow-lg text-success' : 'text-muted'"
                        ...
                        <button type="button" @click="trangThaiFilter = 0"
                            class="btn btn-sm rounded-pill w-50 fw-bold border-0 text-nowrap"
                            :class="trangThaiFilter === 0 ? 'bg-white shadow-lg text-danger' : 'text-muted'"
                            style="transition: all 0.3s ease-in-out; border: 1.5px solid transparent; height: 40px;">
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable">
                ...
                <div class="modal-content border-0 shadow-lg" style="border-radius: 20px; overflow: hidden;">
                    ...
                    <div class="position-relative p-0" style="height: 240px;">
                        <img :src="currentTour.anh ?? default_img" class=" w-100 h-100" style="object-fit: cover;">
                        ...
                        <div class="position-absolute top-0 start-0 m-3">
                            <span
                             ...
                             <span
                            class="badge bg-warning px-3 py-2 rounded-pill shadow-sm text-uppercase border border-white border-opacity-25">
                            <i class="fa-solid fa-circle-check me-1"></i> Tạm đóng
                        </span> 
                        </div>

                        <div class="position-absolute bottom-0 start-0 w-100 p-4">
                            <span class="badge bg-warning text-dark fw-bold mb-2 shadow-sm">{{ currentTour.ma_tour
                            }}</span>
                            <h3 class="text-white fw-bold mb-0 text-shadow" id="suaModalLabel">
                                {{ currentTour.ten_tour }}
                            </h3>
                            <div class="text-white-50 small mt-1">
                                <i class="fa-solid fa-layer-group me-1"></i> Danh mục: {{
                                    currentTour.danh_muc?.ten_danh_muc }}
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card-body p-3">
                                <div class="d-flex justify-content-between align-items-center mb-3">
                                    <h6 class="fw-bold text-primary m-0 text-uppercase small">
                                        <i class="fa-solid fa-calendar-days me-2"></i>Lịch trình:{{ currentTour.gio_di
                                        }}->{{
                                            currentTour.gio_ve }}
                                    </h6>
                                    <span class="text-muted small"><i class="fa-solid fa-map-pin me-1"></i> Quảng
                                        Ninh</span>
                                </div>

                                <div class="d-flex align-items-center justify-content-between px-2 text-center">
                                    <div>
                                        <div class="fw-bold fs-5 text-dark">{{ currentTour.gio_di }}</div>
                                        <div class="text-muted small">{{ currentTour.ngay_di }}</div>
                                    </div>
                                    ...
                                    <div>
                                        <div class="fw-bold fs-5 text-dark">{{ currentTour.gio_ve }}</div>
                                        <div class="text-muted small">{{ currentTour.ngay_ve }}</div>
                                    </div>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="card border-0 shadow-sm rounded-4 h-100">
                                    <div class="card-body p-3">
                                        ...
                                        <div class="d-flex justify-content-between align-items-center mb-2">
                                            <span class="text-muted small"><i class="fa-solid fa-user me-2"></i>Người
                                                lớn</span>
                                            <span class="fw-bold text-primary">{{ currentTour.gia_nguoi_lon }} đ</span>
                                        </div>
                                        <div class="d-flex justify-content-between align-items-center">
                                            <span class="text-muted small"><i class="fa-solid fa-child me-2"></i>Trẻ
                                                em</span>
                                            <span class="fw-bold text-warning">{{ currentTour.gia_tre_em }} đ</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card border-0 shadow-sm rounded-4 h-100">
                                    <div class="card-body p-3 text-center d-flex flex-column justify-content-center">
                                        ...
                                        <div class="d-flex align-items-baseline justify-content-center gap-1">
                                            <span class="fw-bold fs-2 text-success lh-1">{{ currentTour.so_cho_con
                                                }}</span>
                                            <span class="text-muted small">/{{ currentTour.so_cho }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="modal-footer border-top-0 bg-white justify-content-between px-4 py-3">
                        <div class="d-flex align-items-center small text-muted">
                            ...
                            <div>Tạo bởi: <strong class="text-dark">Admin01</strong></div>
                        </div>
...
export default {
    ...
    computed: {
        filteredTours() {
            return this.list_tour.filter(tour => {
                const matchStatus = tour.trang_thai == this.trangThaiFilter;
    ...
    methods: {
        ...
        openDetail(tour) {
            ...
            this.xemChiTietTour(tour.id);
        },

        // API xem chi tiết
        // xemChiTietTour(id) {
        //     axios
        //         .get(`http://127.0.0.1:8000/api/admin/tour/${id}/chi-tiet`, {
        //             headers: {
        //                 Authorization: "Bearer " + localStorage.getItem("auth_token")
        //             }
        //         })
        //         .then(res => {
        //             this.currentTour = res.data.data;
        //         })
